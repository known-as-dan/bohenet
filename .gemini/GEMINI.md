...
> Canonical project instructions live in [`CLAUDE.md`](../CLAUDE.md). This file mirrors the same rules for Gemini CLI.
...
- Server-side test files: `*.{test,spec}.{js,ts}`
...
3. **svelte-autofixer** — run on all Svelte code before finalizing; keep calling until no issues remain
